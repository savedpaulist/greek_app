...
pub fn build_nominal_paradigm(
    ...
    for &num in &numbers {
        for &gen in genders {
            col_headers.push(format!(
                "{} {}",
                num_label(num, lang),
                gender_label(gen, lang)
            ));
            col_keys.push((num, gen));
        }
    }

    let row_headers: Vec<String> = cases
        .iter()
        .map(|c| case_label(c, lang).to_owned())
        .collect();
                    ...
                    let form = forms
                        ...
                        .find(|f| {
                            f.case_tag.as_deref() == Some(case)
                                ...
                                && (genders.len() == 1 || f.gender_tag.as_deref() == Some(gen))
                        })
                        .cloned();
                    ParadigmCell { form }
                })
...
pub fn build_verb_paradigms(
    ...
    let col_headers: Vec<String> = numbers
        .iter()
    ...
    for (tense, mood, voice) in &tmv_set {
        ...
        let persons: Vec<&str> = candidate_persons
            .iter()
            .copied()
        ...
        let row_headers: Vec<String> = persons
            .iter()
            .map(|p| person_label(p, lang).to_string())
            .collect();

        let cells: Vec<Vec<ParadigmCell>> = persons
            .iter()
            .map(|&person| {
                numbers
                    .iter()
    ...
    if !inf_forms.is_empty() {
        ...
        let inf_col_headers: Vec<String> = tv_set
            .iter()
            .map(|(t, v)| format!("{} {}", tense_label(t, lang), voice_label(v, lang)))
            .collect();

        let cells: Vec<Vec<ParadigmCell>> = vec![tv_set
            .iter()
            .map(|(t, v)| {
                let form = inf_forms
                    .iter()
